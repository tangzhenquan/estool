...
func (queryer *Queryer) QueryByTime(ctx context.Context, indexName string, from, to time.Time, size int) ([]QueryRes, error) {
	subCtx, cancel := context.WithTimeout(ctx, time.Second*10)
	defer cancel()
	rows, err := queryer.dao.QueryByTime(subCtx, indexName, from, to, reflect.TypeOf(QueryRes{}), size)
	if err != nil {
		return nil, err
	}
	var res []QueryRes
